...
func TestEthBroadcaster_EthTxInsertEventCausesTriggerToFire(t *testing.T) {
	...
	bulletprooftxmanager.XXXMustStartEthTxInsertListener(eb)

	// Give it some time to start listening
	time.Sleep(100 * time.Millisecond)

	mustInsertUnstartedEthTx(t, store)
	gomega.NewGomegaWithT(t).Eventually(bulletprooftxmanager.XXXTriggerChan(eb)).Should(gomega.Receive())
